Maxim maxim;
// Here I create the audioplayer for the kickbass
AudioPlayer kick;

// This is an array containing booleans for the kick sequence
boolean [] kckSeq;

// This variable tracks how many counts into the beat the loop is.
int playHead;
// This variable keeps track of the position withing the kick
// sequence
int kckSeqPos;
...
void setup(){
  ...
  //Here I create a audio environment.
  maxim = new Maxim(this);
  kick = maxim.loadFile("KickDrum0005.wav");
  frameRate(30);
  playHead = 0;
  
  kckSeq = new boolean[4];
  kckSeq[0] = true;
  kckSeq[1] = false;
  kckSeq[2] = true;
  kckSeq[3] = true;
...
void draw(){
  // This adds one to the playHead for every frame, when 30 frames
  // is reached (= 1 second) it resets.
  playHead ++;
  if (playhead % 30 == 0){
    // This checks if the boolean in the kick sequence is set to true.
    if (kckSeq[kckSeqPos]){
      kick.cue(0);
      kick.play();
    }
    
    // This adds one to the kick sequence positions after every second.
    // When it reaches the end of the array it resets.
    kckSeqPost += 1;
    if (kckSeqPos == kckSeq.length){
      kckSeqPos = 0;
    }
    // This should print out the kick sequence positions. But it doesn't!
    println("positionL: " + kckSeqPos);
  }
}
